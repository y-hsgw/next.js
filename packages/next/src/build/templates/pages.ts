...
export async function handler(
  ...
  if (serverFilesManifest?.config?.experimental) {
    // @ts-expect-error this field's source of truth is the manifest
    nextConfig.experimental.isExperimentalCompile =
  ...
  try {
    ...
    const isNextDataRequest =
      pathHasPrefix(req.url || '/', '/_next/data/') &&
      (hasServerProps || hasStaticProps)
    ...
    const invokeRouteModule = async (span?: Span) =>
      routeModule
        .render(req, res, {
          query:
            hasStaticProps && !nextConfig.experimental.isExperimentalCompile
              ? ({
                  ...params,
