...
> {
  ...
  > {
    ...
    if (process.env.NEXT_RUNTIME !== 'edge') {
      ...
      const [
        ...
        loadManifestFromRelativePath<ReactLoadableManifest>(
          ...
          process.env.TURBOPACK
            ? `server/pages/${normalizedPagePath === '/' ? '/index' : normalizedPagePath}/${REACT_LOADABLE_MANIFEST}`
            : REACT_LOADABLE_MANIFEST
        ),
      ...
      let detectedLocale = getRequestMeta(req, 'locale')
      ...
      if (serverUtils.dynamicRouteMatcher) {
      ...
      const query = {
        // Local "next start" expects these values to not be present
        // in the URL although when deployed proxies will add query
        // values from resolving the routes.
      ...
      for (const [key, value] of Object.entries(parsedUrl.query)) {
        ...
        // Remove the prefixed key (nxtP) from the query params as
      ...
      if (pageIsDynamic) {
        ...
        if (result.hasValidParams) {
          params = Object.assign({}, params, result.params)
